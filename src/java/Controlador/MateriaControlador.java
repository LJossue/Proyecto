...
public class MateriaControlador extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String matCantidad = request.getParameter("matCantidad");
        String matUnidadDeMedida=request.getParameter("matUnidadDeMedida");
        String matEstado = request.getParameter("matEstado");
  
        int opcion = Integer.parseInt(request.getParameter("opcion"));

        MateriaVO matVo = new MateriaVO(matId, matNombre, matCantidad,matUnidadDeMedida, matEstado);
        ...
        switch (opcion) {
            ...
            case 3:
                ...
                if (matDao.eliminarRegistro()) {
                     request.setAttribute("mensajeExito", "El rol de materí prima ha sido actualizado correctamente");
               response.sendRedirect("index.jsp?mensajeExito=El+estado+de+la+materia+prima+se+actualizo+correctamente");
                } else {
                   request.setAttribute("mensajeError", "El rol de materí prima no ha sido actualizado correctamente");
